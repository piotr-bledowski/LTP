...
def create_features_table(dataset_name: str, **kwargs) -> Optional[pd.DataFrame]:
    if not os.path.exists(FEATURES_CACHE_DIR):
        return None

    # Load base features (LDP + node statistics)
    base_filename = f"{dataset_name}_base.zst"
    base_filepath = FEATURES_CACHE_DIR / base_filename
    try:
        df = pd.read_pickle(base_filepath, compression="zstd")
    except FileNotFoundError:
        return None

    # Load each requested topological feature
    true_features = [str(k) for k, v in kwargs.items() if v]
    list_of_features = ["degree_sum", "shortest_paths", "edge_betweenness", "degree_centrality", 
                       "closeness", "local_clustering_coefficient", "pagerank", "eigenvector_centrality", 
                       "algebraic_distance", "diameter", "density", "preferential_attachment", 
                       "common_neighbor", "katz_index", "jaccard_index", "adjusted_rand", 
                       "adamic_adar", "local_degree_score", "local_similarity_score", "scan"]
    
    for feature in true_features:
        filename = f"{dataset_name}_{feature}.zst"
        filepath = FEATURES_CACHE_DIR / filename
        try:
...
def cache_features(features: pd.DataFrame, dataset_name: str, is_base: bool = False, feature_name: str = None, **kwargs) -> None:
    FEATURES_CACHE_DIR.mkdir(exist_ok=True)

    if is_base:
        # Save base features (LDP + node statistics)
        filename = f"{dataset_name}_base.zst"
    else:
        # Save individual topological feature
        filename = f"{dataset_name}_{feature_name}.zst"
    
    filepath = FEATURES_CACHE_DIR / filename
    features.to_pickle(filepath, compression={"method": "zstd", "threads": -1}, protocol=5)
